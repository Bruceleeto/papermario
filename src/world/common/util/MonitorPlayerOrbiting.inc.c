...
API_CALLABLE(N(MonitorPlayerOrbiting)) {
    ...
    switch (orbit->state) {
        ...
        case ORBIT_STATE_SECOND_FRAME:
            ...
            dist = get_xz_dist_to_player(orbit->pos.x, orbit->pos.z);
            if (dist < orbit->startRadius) {
                prevAngle = pm_atan2(orbit->pos.x, orbit->pos.z, orbit->firstPlayerX, orbit->firstPlayerZ);
                curAngle = pm_atan2(orbit->pos.x, orbit->pos.z, playerStatus->pos.x, playerStatus->pos.z);
        ...
        case ORBIT_STATE_STARTING_ORBIT:
            dist = get_xz_dist_to_player(orbit->pos.x, orbit->pos.z);
            if (dist < orbit->startRadius) {
                prevAngle = pm_atan2(orbit->pos.x, orbit->pos.z, orbit->lastPlayerX, orbit->lastPlayerZ);
                curAngle = pm_atan2(orbit->pos.x, orbit->pos.z, playerStatus->pos.x, playerStatus->pos.z);
                deltaAngle = get_clamped_angle_diff(prevAngle, curAngle);
