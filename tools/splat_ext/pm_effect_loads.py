...
"""
    ...
    @staticmethod
    def get_effect_c(index, effect: "Effect"):
        if effect.empty:
            return ""

        name = effect.name
        args = effect.args
        returns_void = effect.returns_void

        ret_type = "void" if returns_void else "EffectInstance*"

        # Parse args to generate arg names
        if args.strip():
            arg_types = [a.strip() for a in args.split(',')]
            arg_names = [f"arg{i}" for i in range(len(arg_types))]
            arg_list = ', '.join(f"{t} {n}" for t, n in zip(arg_types, arg_names))
            call_args = ', '.join(arg_names)
        else:
            arg_list = "void"
            call_args = ""

        ret_stmt = "" if returns_void else "return "

        return f"""{ret_type} fx_{name}({arg_list}) {{
    load_effect({index});
    {ret_stmt}{name}_main({call_args});
}}

"""

    @staticmethod
    def get_effect_c_header(effects: List["Effect"]):
        header = """// Auto-generated effect wrappers for non-N64 platforms
// Do not edit manually - generated by pm_effect_loads.py
...
// Forward declarations for *_main functions
"""
        for effect in effects:
            if not effect.empty:
                ret_type = "void" if effect.returns_void else "EffectInstance*"
                header += f"extern {ret_type} {effect.name}_main({effect.args});\n"
    ...
    def effect_c_path(self):
        return options.opts.build_path / "src" / "fx_wrappers.c"

    def split(self, rom_bytes):
        # Generate .s files for N64
        ...
        # Generate single .c file for PC/other platforms
        self.effect_c_path().parent.mkdir(parents=True, exist_ok=True)

        with open(self.effect_c_path(), "w") as f:
            f.write(N64SegPm_effect_loads.get_effect_c_header(self.effects))
            for i, effect in enumerate(self.effects):
                f.write(N64SegPm_effect_loads.get_effect_c(i, effect))
